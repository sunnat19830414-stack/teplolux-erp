...
<?php foreach ($m['by_currency'] as $cur => $v): ?>
  <div class="card">
    ...
    <div class="kpi-grid">
      ...
      <?php if (abs($v['owner']) > 0.004): ?>
      <div class="kpi"><div class="k">Собственник: вложил − забрал</div>
        <div class="v"><?= ($v['owner'] > 0 ? '+' : '') . number_format($v['owner'], 2, '.', ' ') ?> <?= htmlspecialchars(cur_label($cur)) ?></div>
        <div class="muted">не доход и не расход</div></div>
      <?php endif; ?>
    </div>
  </div>
...
<div class="card">
  ...
  всё, что списано — «ушло». Переводы между своими счетами и вложения/изъятия собственника сюда не
  входят — это не выручка и не расход компании, они показаны отдельно. Валюты не смешиваются: сумовый счёт считается в сумах, евровый в евро.
