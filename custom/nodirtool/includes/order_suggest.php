...
// ── NodirTool (11.09.2026): расчёт тот же, что у шефа в BossTool (копия его includes/order_suggest.php).
// Вспомогательные функции BossTool (stock_lookup.php) здесь частично есть под теми же именами в
// product_lookup.php / currency.php, поэтому недостающие объявляем только если их ещё нет.
if (!function_exists('stock_lookup_db')) {
    function stock_lookup_db(): mysqli { return product_lookup_db(); }
}
if (!function_exists('supplier_contract_currency')) {
    function supplier_contract_currency(int $supplierId): string
    {
        if ($supplierId <= 0) return 'USD';
        $st = stock_lookup_db()->prepare("SELECT multicurrency_code FROM llx_societe WHERE rowid = ?");
        $st->bind_param('i', $supplierId); $st->execute();
        $row = $st->get_result()->fetch_assoc(); $st->close();
        return strtoupper(trim((string)($row['multicurrency_code'] ?? ''))) ?: 'USD';
    }
}
if (!function_exists('currency_rate')) {
    /** Свежий курс валюты из справочника Dolibarr (единиц за 1 доллар). */
    function currency_rate(string $code): float
    {
        $code = strtoupper(trim($code));
        if ($code === '' || $code === 'USD') return 1.0;
        $st = stock_lookup_db()->prepare("SELECT r.rate FROM llx_multicurrency_rate r JOIN llx_multicurrency m ON m.rowid = r.fk_multicurrency
