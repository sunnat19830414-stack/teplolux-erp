<?php
/**
 * NodirTool (11.09.2026): та же страница, что у шефа в BossTool (request_view.php), для заявок,
 * составленных закупщиками. Заявки руководства открываются в «Заявки к оформлению».
 *
 * Одна заявка: выбор поставщика, набор позиций, отправка в работу. Пока заявка черновик — её можно
...
$directions = ['J' => 'Жоми', 'T' => 'Турк'];
$allDirs = array_keys($directions);
...
// Заявки руководства здесь не редактируются — их место в «Заявки к оформлению».
if (!in_array($req['created_by'], array_keys($cfg['users']), true)) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$isDraft && in_array($action, ['add_lines', 'update_line', 'delete_line', 'save_head', 'send'], true)) {
        $message = 'Заявка уже отправлена в работу — менять её нельзя. Создайте новую.';
        $messageType = 'err';
        ...
        $allowed = supplier_order_suggestions($supId, $allDirs);
        $byId = [];
        ...
        if ($supId > 0) {
            ...
            foreach ($prices as $pid => $raw) {
                ...
                // та же запись, что при оформлении заказа: справочник Dolibarr + журнал закупочных цен
                if (save_purchase_price_with_history($api, $pid, $supId, $newPrice, $_SESSION['user']['name'] ?? '', $suppCurSave, $rate)) {
                    $priceSaved++;
                } else {
        ...
        if (empty($req['lines'])) {
            ...
            flash_set('Заявка отправлена — она в «Заявки к оформлению» у Нодира и Абдурашида. Возьмите её там в работу, чтобы собрать заказ.', 'ok');
            header('Location: request_view.php?id=' . $id);
            exit;
        }
...
<?php if ($req['status'] === 'declined' && $req['decline_reason']): ?>
  <p class="warn">Заявку отклонили: <?= htmlspecialchars($req['decline_reason']) ?></p>
<?php endif; ?>
...
<div class="card req-head">
  ...
  <div class="req-head-foot">
    <span class="muted">
      Направление: <?= htmlspecialchars($directions[$req['direction']] ?? $req['direction']) ?>
      · создана <?= date('d.m.Y', strtotime($req['created_at'])) ?> (<?= htmlspecialchars($cfg['users'][$req['created_by']]['display_name'] ?? $req['created_by']) ?>)
      · в заявке позиций: <strong><?= $lineCount ?></strong>
    </span>
    <?php if ($isDraft): ?>
      <span class="req-head-actions">
        <form method="post" onsubmit="return appConfirmSubmit(this, 'Отправить заявку в работу? После отправки менять её будет нельзя.');">
        <?= csrf_field() ?>
          <input type="hidden" name="action" value="send">
          <input type="hidden" name="id" value="<?= $id ?>">
          <button type="submit" <?= $lineCount === 0 ? 'disabled' : '' ?>>Отправить в работу</button>
        </form>
...
<?php if ($isDraft): ?>
  ...
  <?php else: ?>
    <?php
      ...
      $suggData = supplier_order_suggestions((int)$req['fk_supplier'], $allDirs, $monthsTarget);
      $sugg = $suggData['rows'];
    ...
    <div class="card">
      ...
      <?php if (empty($sugg)): ?>
        <p class="muted">У этого поставщика нет товаров в каталоге<?php
          if (count($allDirs) === 1) echo ' по вашему направлению'; ?>.</p>
      <?php else: ?>
        <?php
        ...
        <?= csrf_field() ?>
          ...
          <div class="sugg-foot">
            ...
            <div class="muted">
              ...
              Средний расход считается <?= htmlspecialchars(sales_history_note()) ?>
              (глубина расчёта — <?= rtrim(rtrim(number_format($window, 1, '.', ''), '0'), '.') ?> мес.).
...
<div class="card">
  ...
  <?php if ($isDraft): ?>
    <p style="margin:-4px 0 12px">
      <a class="btn secondary small" href="product_form.php?ctx=request&request_id=<?= $id ?>">+ Новый товар</a>
